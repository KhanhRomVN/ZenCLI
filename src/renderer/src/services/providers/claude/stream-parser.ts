import type { ContentBlock, Citation, Step } from '../../../types/providers/claude'
...
export function buildSteps(contentBlocks: Map<number, ContentBlock>): Step[] {
  ...
  const sortedBlocks = Array.from(contentBlocks.entries()).sort((a, b) => a[0] - b[0])

  for (const [idx, block] of sortedBlocks) {
    if (
      block.type === 'thinking' &&
      (block.thinking || block.thinkingText || block.summaries?.length)
    ) {
...
function processSSEEvent(
      ...
      if (deltaType === 'input_json_delta' && delta?.partial_json) {
        ...
        try {
          ...
          if (
            ...
            parsed.length > 0 &&
            parsed.every((d: unknown) => typeof d === 'object' && d !== null && 'type' in d)
          ) {
  ...
  if (data.type === 'tool_approval') {
    callbacks.onToolApproval?.(data.tool_name as string, data.approval_key as string, data.input)
  }
