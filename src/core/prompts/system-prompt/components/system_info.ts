...
export async function getSystemEnv(
  ...
  const workspaces = [currentWorkDir];
  return isTesting
    ? {
        os: "macOS",
        ide: "TestIde",
...
export async function getSystemInfo(
  ...
  const template =
    variant.componentOverrides?.[SystemPromptSection.SYSTEM_INFO]?.template ||
    SYSTEM_INFO_TEMPLATE_TEXT;
