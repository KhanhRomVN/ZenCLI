...
export class Task {
  ...
  constructor(params: TaskParams) {
    ...
    if ((global as any).standaloneTerminalManager) {
      ...
      if (terminalExecutionMode === "backgroundExec") {
        try {
          const { StandaloneTerminalManager } =
            require(Task.STANDALONE_TERMINAL_MODULE_PATH) as {
          ...
          console.error(
            "[DEBUG] Failed to load standalone terminal manager",
            error
          );
          this.terminalManager = new TerminalManager();
        }
  ...
  async executeCommandTool(
    ...
    if (isSubagent) {
      ...
      terminalManager.setShellIntegrationTimeout(
        this.terminalManager["shellIntegrationTimeout"] || 4000
      );
      terminalManager.setTerminalReuseEnabled(
        this.terminalManager["terminalReuseEnabled"] ?? true
      );
      terminalManager.setTerminalOutputLineLimit(
        this.terminalManager["terminalOutputLineLimit"] || 500
      );
    ...
    const clearCommandState = async () => {
      ...
      const clineMessages = this.messageStateHandler.getClineMessages();
      const lastCommandIndex = findLastIndex(
        clineMessages,
        (m) => m.ask === "command" || m.say === "command"
      );
    ...
    const flushBuffer = async (force = false) => {
      ...
      try {
        ...
        if (response === "yesButtonClicked") {
          ...
          telemetryService.captureTerminalUserIntervention(
            TerminalUserInterventionAction.CANCELLED
          );
          didCancelViaUi = true;
    ...
    completionTimer = setTimeout(() => {
      if (!completed) {
        telemetryService.captureTerminalHang(
          TerminalHangStage.WAITING_FOR_COMPLETION
        );
    ...
    process.once("no_shell_integration", async () => {
      const shouldShowSuggestion =
        this.controller.shouldShowBackgroundTerminalSuggestion();
    ...
    const result = terminalManager.processOutput(
      outputLines,
      isSubagent
        ? terminalManager["subagentTerminalOutputLineLimit"]
        : undefined,
      isSubagent
    );
    ...
    try {
      ...
      if (typeof process.terminate === "function") {
        try {
          ...
          Logger.error(
            `Error terminating background command: ${command}`,
            error
          );
        }
      }
      ...
      if (typeof process.continue === "function") {
        try {
          ...
          Logger.error(
            `Error continuing background command: ${command}`,
            error
          );
        }
      }
      ...
      const processedOutput = this.terminalManager.processOutput(
        outputLines,
        undefined,
        false
      );
      ...
      try {
        await this.say(
          "command_output",
          "Command execution has been cancelled."
        );
      } catch (error) {
  ...
  async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
    ...
    const promptContext: SystemPromptContext = {
      ...
      yoloModeToggled:
        this.stateManager.getGlobalSettingsKey("yoloModeToggled"),
      clineWebToolsEnabled: this.stateManager.getGlobalSettingsKey(
    ...
    const useResponseApi =
      this.useNativeToolCalls && featureFlagsService.isResponseApiEnabled();
  ...
  async recursivelyMakeClineRequests(
    ...
    let [parsedUserContent, environmentDetails, clinerulesError] =
      await this.loadContext(userContent, includeFileDetails, useCompactPrompt);
          ...
          switch (chunk.type) {
            ...
            case "tool_calls": {
              ...
              if (
                ...
                if (
                  chunk.tool_call.function.name.includes(
  ...
  async getEnvironmentDetails(includeFileDetails: boolean = false) {
    ...
    details += `\n\n# Current Time\n${formatter.format(
      now
    )} (${timeZone}, UTC${timeZoneOffsetStr})`;
