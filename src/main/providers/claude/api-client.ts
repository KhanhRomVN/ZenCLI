...
import type {
  ...
  UploadFilePayload,
  AccountInfo
} from '../../../renderer/src/types/providers/claude'
...
export async function getAvailableModels(orgId: string): Promise<any[]> {
  const url = `${BASE_URL}/api/organizations/${orgId}/models`
  const result = await makeRequest(url, 'GET')

  if (result.status !== 200) {
    throw new Error(`Failed to get models: ${result.status}`)
  }
